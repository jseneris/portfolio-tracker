...
### PLAN
    ...
    [x] 5. Add a target management page in frontend for editing all rule inputs.
...
### PLAN
    [x] 1. Add closing-price table schema with unique ticker and date constraint plus source metadata.
    [~] 2. Build idempotent backend ingestion logic for daily end-of-day closing prices.
    [x] 3. Add utility endpoints to backfill historical closes from earliest holding date for each ticker.
    [ ] 4. Add utility page to trigger and monitor historical backfill jobs.
    [x] 5. Implement query endpoints to retrieve historical closes by ticker and date range.
...
### PLAN
    ...
    [x] 4. Build backend comparison endpoint returning aligned time series for portfolio and indexes.
