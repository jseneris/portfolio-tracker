...
The `POST /api/lots/:ticker/split` endpoint accepts `ratioNumerator` and `ratioDenominator` (e.g. `{ ratioNumerator: 2, ratioDenominator: 1 }` for a 2-for-1 split, `{ ratioNumerator: 5, ratioDenominator: 3 }` for a 5-for-3 split) instead of a single pre-computed `multiplier`.
