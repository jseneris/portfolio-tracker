...
- **unitCost**: Cost per share, `DECIMAL(18,8)`
...
### StockSplits
Global record of each stock split for a ticker. A given ticker can have any number of `StockSplits` rows (one per split event); the same `(ticker, ratioNumerator, ratioDenominator, splitDate)` combination cannot be applied twice.
- **id**: Unique identifier
- **ticker**: Stock ticker symbol
...
- **splitDate**: Effective date of the split
- **createdAt**: Timestamp
- **validation constraints**:
	- `ratioNumerator > 0`
	- `ratioDenominator > 0`
